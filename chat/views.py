...
#     api_key="WTwXnFzNmtBjaOPzqBmW4En3xIBo50qk",
#     base_url="https://api.mistral.ai/v1"
# )

# # -- UNCENSORED Client (e.g., Dolphin, MythoMax etc. hosted by aimlapi.com) --
# uncensored_client = OpenAI(
#     api_key="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",  # Replace with your actual OpenRouter key
...
#     prefix = "uncensored-" if mode == "uncensored" else ""
#     sid = _new_session_id(prefix)
#     ChatSession.objects.create(session_id=sid, mode=mode)
#     return Response({"session_id": sid, "title": "New chat", "mode": mode}, status=201)

# # -- Standard Chat (Mistral) --
...
#             ChatSession.objects.create(session_id=session_id, mode=mode)

#         # Save user message
#         Message.objects.create(role="user", content=user_message, session_id=session_id, mode=mode)

#         # Title the chat on first message (once)
#         cs = ChatSession.objects.filter(session_id=session_id).first()
#         if cs and not cs.title:
#             snippet = Truncator(user_message.strip()).words(6, truncate="…")
#             cs.title = snippet or "New chat"
#             cs.save(update_fields=["title", "updated_at"])

#         # Build history & call model (unchanged aside from clients/model names you already have)
#         history = Message.objects.filter(session_id=session_id, mode=mode).order_by("-timestamp")[:10][::-1]
#         messages = [{"role": m.role, "content": m.content} for m in history]
#         client = uncensored_client if mode == "uncensored" else mistral_client
#         model = ("cognitivecomputations/dolphin-mistral-24b-venice-edition:free"
#                  if mode == "uncensored" else "mistral-small")

#         response = client.chat.completions.create(model=model, messages=messages)
#         reply = response.choices[0].message.content

#         Message.objects.create(role="assistant", content=reply, session_id=session_id, mode=mode)
#         return Response({"response": reply, "session_id": session_id})
...
# class UncensoredChatView(APIView):
#     def post(self, request):
#         user_message = request.data.get("message")
#         session_id = request.data.get("session_id", "uncensored")
...
#         try:
#             # Save user message
#             Message.objects.create(role="user", content=user_message, session_id=session_id)

#             # Prepare messages for context (optional, if you want context to model)
#             history = Message.objects.filter(session_id=session_id).order_by("timestamp")[:10]
#             messages = [{"role": m.role, "content": m.content} for m in history]

#             # If you want *only* the latest, you could just:
...
#             Message.objects.create(role="assistant", content=reply, session_id=session_id)

#             return Response({"response": reply})

#         except Exception as e:
#             print("Uncensored error:", e)
#             return Response({"error": "Uncensored server error"}, status=500)
...
#     # Join last activity from Message timestamps
#     last_map = (
#         Message.objects.values("session_id")
...
#     # 2) delete the session record (removes the sidebar tab)
